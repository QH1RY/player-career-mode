(() => {
  ...
  function passToward(from,to,speed=5.0){
    const dx=to.x-from.x,dy=to.y-from.y,d=Math.hypot(dx,dy)||1;
    ballFree(from.x,from.y,dx/d*speed,dy/d*speed);
  }

  function bestSupportTarget(){
    const p=match.player,dirX=lastMoveX||1,dirY=lastMoveY||0;
    let best=null,bestScore=-9999;
    match.mates.forEach((a,i)=>{
      if(!a)return;
      const dx=a.x-p.x,dy=a.y-p.y,dist=Math.hypot(dx,dy)||1;
      const forward=(dx*dirX+dy*dirY)/dist;
      const width=Math.abs(dy)/220;
      const pressure=match.foes.reduce((m,f)=>Math.min(m,Math.hypot(f.x-a.x,f.y-a.y)),999);
      const score=forward*2.2-Math.abs(dist-145)/130+Math.min(1,pressure/90)*.9+width*.15+(i>=6?.25:0);
  ...
  function passBall(){
    if(!match)return;
    if(match.ball.owner==='mate'&&match.ball.carrier){
      const a=match.ball.carrier;passToward(a,match.player,5.3);flash('Teammate returns it.');return;
    }
    if(match.ball.owner!=='player')return;
    const target=bestSupportTarget();
    if(!target)return;
    passToward(match.player,target,5.25);match.chances++;match.rating=Math.min(10,match.rating+.08);flash('Pass played.');
  }
  ...
  function moveMate(a,tx,ty,speed){
    const dx=tx-a.x,dy=ty-a.y,d=Math.hypot(dx,dy)||1;
    const step=Math.min(speed,d);
    a.x+=dx/d*step;a.y+=dy/d*step;
    a.x=Math.max(40,Math.min(1040,a.x));a.y=Math.max(40,Math.min(580,a.y));
  }

  function updateTeammates(){
    const p=match.player,b=match.ball;
    if(!match.mates.length)return;
    match.mates.forEach((a,i)=>{if(a.baseLane==null){a.baseLane=(i%5);a.role=i<3?'defender':i<6?'midfielder':'attacker';a.supportPhase=Math.random()*Math.PI*2;}});

    const nearestFoeDist=match.foes.reduce((m,f)=>Math.min(m,Math.hypot(f.x-p.x,f.y-p.y)),999);
    const underPressure=nearestFoeDist<78;
    const nearestMateToBall=match.mates.reduce((best,a)=>{const d=Math.hypot(a.x-b.x,a.y-b.y);return d<(best?.d??999)?{a,d}:best},null);

    match.mates.forEach((a,i)=>{
      let tx=a.x,ty=a.y,speed=1.0;
      const laneY=70+(i%5)*120;

      if(b.owner==='player'){
        if(a.role==='attacker'){
          tx=Math.min(1015,p.x+150+(i-6)*42);
          ty=Math.max(65,Math.min(555,p.y+(i===6?-125:i===7?0:125)));
          speed=1.28;
        }else if(a.role==='midfielder'){
          if(underPressure&&i===3){tx=p.x-55;ty=p.y-70;speed=1.35;}
          else if(underPressure&&i===4){tx=p.x-50;ty=p.y+70;speed=1.35;}
          else{tx=Math.min(960,p.x+65+(i-3)*25);ty=p.y+(i===3?-105:i===4?0:105);speed=1.15;}
        }else{
          tx=Math.max(90,p.x-145+(i%2)*35);ty=laneY;speed=.92;
        }
      }else if(b.owner==='mate'){
        if(b.carrier===a){
          tx=Math.min(1005,a.x+85);ty=a.y+(310-a.y)*.06;speed=1.38;
        }else if(a.role==='attacker'){
          tx=Math.min(1030,b.x+120);ty=laneY;speed=1.22;
        }else{
          tx=Math.min(980,b.x+20+(i%3)*45);ty=laneY;speed=1.0;
        }
      }else if(b.owner===null&&nearestMateToBall?.a===a){
        tx=b.x;ty=b.y;speed=1.48;
      }else{
        tx=Math.min(930,220+(i%3)*170);ty=laneY;speed=.86;
      }

      moveMate(a,tx,ty,speed);
    });

    if(b.owner===null&&nearestMateToBall&&nearestMateToBall.d<21&&Math.hypot(b.vx||0,b.vy||0)<4.5){
      b.owner='mate';b.carrier=nearestMateToBall.a;b.vx=b.vy=0;b.carrier.holdUntil=performance.now()+650+Math.random()*500;
    }

    if(b.owner==='mate'&&b.carrier){
      const a=b.carrier;b.x=a.x+12;b.y=a.y;
      const pressure=match.foes.reduce((m,f)=>Math.min(m,Math.hypot(f.x-a.x,f.y-a.y)),999);
      const canRelease=performance.now()>(a.holdUntil||0);
      if(canRelease&&(pressure<65||Math.hypot(a.x-p.x,a.y-p.y)>230||Math.random()<.012)){
        if(p.x<a.x+120){passToward(a,p,5.25);flash('Teammate finds you.');}
        else{
          const forward=match.mates.filter(x=>x!==a&&x.x>a.x+40).sort((x,y)=>y.x-x.x)[0];
          if(forward){passToward(a,forward,5.0);}else{passToward(a,p,5.1);}
        }
      }
    }
  }
  ...
  update=function(){
    ...
    if(b.owner===null&&Math.hypot(p.x-b.x,p.y-b.y)<23&&Math.hypot(b.vx||0,b.vy||0)<4.8){b.owner='player';b.carrier=null;b.vx=b.vy=0;}
    if(b.owner==='player'){b.x=p.x+16*(lastMoveX||1);b.y=p.y+8*(lastMoveY||0);}else if(b.owner==='foe'&&b.carrier){b.x=b.carrier.x-13;b.y=b.carrier.y;}else if(b.owner===null){b.x+=(b.vx||0);b.y+=(b.vy||0);b.vx=(b.vx||0)*.985;b.vy=(b.vy||0)*.985;if(!match.shotInFlight){if(b.y<25||b.y>595)b.vy*=-.75;if(b.x<25||b.x>1075)b.vx*=-.75;}}
